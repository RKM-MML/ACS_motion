...
def focus_stack_handler():
    folder_path = filedialog.askdirectory(title="Select Folder with Focus Stack Images")
    if not folder_path:
        return
    try:
    ...
    def processing_thread():
        try:
            _, point_cloud_data, _ = process_focus_stack(
                images,
            ...
            point_cloud, colors = point_cloud_data
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(point_cloud)
            pcd.colors = o3d.utility.Vector3dVector(colors)
            o3d.visualization.draw_geometries([pcd])
        except Exception as e:
            root.after(0, messagebox.showerror, "Processing Error", f"Error during focus stacking: {e}")
